...
{  
    ...
    while(cin>>n)  
    {  
        if(n==1){cout<<"is not a D_num 1"<<endl;continue;}//特判  
        t=0;  
        find(n);  
        if(t!=2&&t!=3){cout<<"is not a D_num 2"<<endl;continue;}  
        sort(f,f+t);  
        if(t==2)  
        {  
            if(f[0]!=f[1])cout<<f[0]<<" "<<f[1]<<" "<<n<<endl;  
            else cout<<"is not a D_num 3"<<endl;  
        }  
        else//n是一个素数的三次方  
        {  
            cout << f[0] << " " << f[1] << " " << f[2] << endl << endl;
            if(f[0]==f[1]&&f[1]==f[2])cout<<f[0]<<" "<<f[0]*f[0]<<" "<<n<<endl;  
            else cout<<"is not a D_num 4"<<endl;  
        }  
    }  
    return 0;  
}  